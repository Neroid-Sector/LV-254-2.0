...
/datum/emergency_call/inspection_tis/create_member(datum/mind/M, turf/override_spawn_loc)
	...
	if(!leader && HAS_FLAG(H.client.prefs.toggles_ert, PLAY_LEADER))
		leader = H
		arm_equipment(H, /datum/equipment_preset/uaac/tis/io, TRUE, TRUE)
...
/datum/emergency_call/inspection_tis_senior/create_member(datum/mind/M, turf/override_spawn_loc)
	...
	if(!leader && HAS_FLAG(H.client.prefs.toggles_ert, PLAY_LEADER))
		leader = H
		arm_equipment(H, /datum/equipment_preset/uaac/tis/sio, TRUE, TRUE)
...
/datum/emergency_call/inspection_tis_coordinator/spawn_items()
	var/turf/drop_spawn
	drop_spawn = get_spawn_point(TRUE)
	new /obj/item/storage/box/handcuffs(drop_spawn)
	new /obj/item/storage/box/handcuffs(drop_spawn)
